...
const selected_option = document.querySelector('.phone-input .selected-option div');
const verificationInput = document.querySelector(".verification-input input");
const verificationInputDiv = document.querySelector(".verification-input");
let submit_stage = 0;
...
// Insert country options into the telephone input list
for (const country of countries) {
    const option = `
    <li class="option">
        <div>
            <span class="iconify" data-icon="flag:${country.code.toLowerCase()}-1x1"></span>
            <span class="country-name">${country.name}</span>
        </div>
        <strong>+${country.phone}</strong>
    </li>`;
    document.querySelector('.phone-input .select-box ol').insertAdjacentHTML('beforeend', option);
    options = document.querySelectorAll('.phone-input .option');
}

function selectOption() {
    // Get the new flag icon data value from the clicked option.
    const newIconData = this.querySelector('.iconify').getAttribute('data-icon'),
        phone_code = this.querySelector('strong').innerText;

    // Find the existing flag icon in the selected option.
    const flagIcon = selected_option.querySelector('.flag-container .iconify');
    if (flagIcon) {
        // Update the data attribute so that Iconify loads the new flag.
        flagIcon.setAttribute('data-icon', newIconData);
    }

    const prefix = phone_code + " ";
    input_box.value = prefix;
    input_box.setAttribute('data-prefix', prefix);
    select_box.classList.remove('active');
    selected_option.classList.remove('active');
    search_box.value = '';
    document.querySelectorAll('.phone-input .select-box ol li.hide')
...
// Toggle options on clicking the selected option area
document.querySelector('.phone-input .selected-option div').addEventListener('click', () => {
    select_box.classList.toggle('active');
    document.querySelector('.phone-input .selected-option div').classList.toggle('active');
});

// Attach event listeners to each option and to the search box
...
input_box.addEventListener('input', function (e) {
    ...
    // Allow only numerical input after the prefix.
    let suffix = this.value.substring(prefix.length);
    suffix = suffix.replace(/\D/g, '');  // Remove any non-digit characters.
...
function isValidPhoneNumber(phoneNumber, options = {}) {
    ...
    // Ensure the phone number consists only of digits.
    if (!/^\d+$/.test(phoneNumber)) {
        return false;
    }

    // Check if the length is within the allowed range.
    if (phoneNumber.length < settings.minLength || phoneNumber.length > settings.maxLength) {
        return false;
    }
